...
- user/admin UI visibility rules;
...
- data statuses визуально отделены от основного ответа;
- document draft отображается как проект, требующий проверки;
- structured JSON можно открыть и скопировать;
- user-facing UI на русском языке;
- icon-first/contextual hints используются там, где помогают не перегружать интерфейс;
- critical warnings не скрыты только за иконками;
