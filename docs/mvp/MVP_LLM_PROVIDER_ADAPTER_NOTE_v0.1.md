...
`LLM_MODEL=<GEMINI_FLASH_MODEL_ID>` и `LLM_API_KEY=<GEMINI_API_KEY>` являются placeholders. Adapter не должен принимать placeholder model id или placeholder API key как валидные values.
