...
| `LLM_MODEL` | Да | `<GEMINI_FLASH_MODEL_ID>` | Имя модели. Для MVP ожидается модель семейства Gemini Flash, но exact id задается через env. |
...
Для первой реализации ожидается `LLM_PROVIDER=gemini`. Точный Gemini Flash model id остается placeholder `LLM_MODEL=<GEMINI_FLASH_MODEL_ID>` и должен быть подтвержден на момент реализации. `LLM_API_KEY` не коммитится и передается только через реальный runtime env или secrets layer.
...
- `LLM_MODEL=<GEMINI_FLASH_MODEL_ID>` остается placeholder до проверки актуального Gemini model id и не может использоваться как реальная модель.
